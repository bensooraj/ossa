...
class Ossa extends EventEmitter {
     ...
     * @param {Number} [options.mode=0] Set the mode
     *  <pre>
     *    '0'    Send and receive
     *    '1'    Send only
     *  </pre>
    ...
    async sendNotification(notificationPayload) {
        ...
        _log("notificationPayload received: %O", notificationPayload);
        ...
        if (notificationPayload.hasOwnProperty("on") && notificationPayload.on !== "") {
            ...
            if (!moment(notificationPayload.on).isValid()) {
                throw new Error(`[notificationPayload.on] Invalid date format: ${notificationPayload.on}`);
            }
            ...
            expirationInMilliSeconds = moment(notificationPayload.on).unix();
        ...
        // Set key with expiry only IF IT DOESN'T EXIST already
        if (notificationPayload.hasOwnProperty("on")) {
            multi.set(notificationID, moment().utc().format("YYYY-MM-DD HH:mm:ss"), "NX");
            multi.expireat(notificationID, expirationInMilliSeconds);

        } else if (notificationPayload.hasOwnProperty("in")) {
            multi.set(notificationID, moment().utc().format("YYYY-MM-DD HH:mm:ss"), "PX", expirationInMilliSeconds, "NX");
        }
        // Store the payload
        multi.hset(this.ossa_payload_store, notificationID, JSON.stringify(notificationPayload));

        // Store a copy of the notificationID in a Redis set
        multi.sadd(this.ossa_processing_queue, notificationID);
    ...
    async deleteNotification(notificationID) {
        ...
        multi.srem(this.ossa_processing_queue, notificationID);
    ...
    async keyspaceNotificationsHandler(channel, message) {
        ...
        if (
            ...
            const sremAsync = promisify(this.client.srem).bind(this.client);
            try {
                const notificationID = message;
                const notificationIDExists = await sremAsync(this.ossa_processing_queue, message);
                if (notificationIDExists) {
                    _log("[sremAsync]: %s", notificationIDExists);
                    const multi = this.client.multi();
                    multi.hget(this.ossa_payload_store, notificationID);
                    multi.hdel(this.ossa_payload_store, notificationID);
                    const multiExecAsync = promisify(multi.exec).bind(multi);
