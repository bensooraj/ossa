...
class Hermes extends EventEmitter {
    constructor(options) {
        ...
        this.subscriber.on('message', this.handleExpiredEvent.bind(this));

        // Subscriber | For recieving
        this.subscriber = this.client.duplicate()
    }
    ...
    async notify(options = {}) {
        ...
        const key = `${this.namespace}::${uuidv4()}`;
        // Set key with expiry
        multi.setex(key, expirationDurationInSeconds, new Date().toTimeString());
        // Store the payload
        multi.hset(this.hermes_payload_store, key, options.payload);
    ...
    async handleExpiredEvent(channel, message) {
